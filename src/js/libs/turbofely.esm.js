...
export class TurboFeLy {
  ...
  isSimulatedClick(event) {
    return (
      Date.now() - this.lastTouchTime < this.options.preventClickDelay ||
  ...
  executeDiffUpdate(oldContent, newContent) {
    try {
      ...
      this.diffUpdate(
        oldContent,
        tempWrapper.firstElementChild,
        oldContent.parentNode,
      );
  ...
  shouldReplaceNodes(oldNode, newNode) {
    ...
    if (oldNode instanceof HTMLInputElement) {
      if (oldNode.type !== newNode.type || oldNode.checked !== newNode.checked)
        return true;
    }
  ...
  syncAttributes(oldNode, newNode) {
    Array.from(oldNode.attributes).forEach((attr) => {
      if (
        ...
        !this.options.ignoreAttributes.includes(attr.name)
      ) {
    ...
    Array.from(newNode.attributes).forEach((attr) => {
      if (!this.options.ignoreAttributes.includes(attr.name)) {
        const normalizedOld = this.normalizeAttributeValue(
          oldNode.getAttribute(attr.name) || "",
          attr.name,
        );
        const normalizedNew = this.normalizeAttributeValue(
          attr.value,
          attr.name,
        );
  ...
  handleDiffError(error, oldContent, newContent) {
    ...
    if (this.options.debug) {
      console.log(
        "[TurboFeLy Debug] Old node structure:",
        oldContent?.cloneNode(true),
      );
      console.log(
        "[TurboFeLy Debug] New node structure:",
